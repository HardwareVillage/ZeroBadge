...
static int check_data(
    char pattern[], int iterations, 
    int tck, int tdi, int tdo,
    int *reg_len)
{
    int i = 0, w = 0, plen = strlen(pattern);    
    char tdo_read = '\0', tdo_prev = '\0';    
...
{
    int tck = 0, tms = 0, tdo = 0, tdi = 0, ntrst = 0;
    int len = 0, reg_len = 0, checkdataret = 0;
    ...
    for (ntrst = 0; ntrst < pinslen; ntrst += 1) 
    {
        for (tck = 0; tck < pinslen; tck += 1) 
        {
            if (tck == ntrst) continue;

            for (tms = 0; tms < pinslen; tms += 1) 
            {
                if (tms == ntrst || tms == tck) continue;

                for (tdo = 0; tdo < pinslen; tdo += 1) 
                {
                    if (tdo == ntrst || tdo == tck || tdo == tms) continue;

                    for (tdi = 0; tdi < pinslen; tdi += 1) 
                    {
                        if (tdi == tck || tdi == tms ||
                            tdi == tdo || tdi == ntrst) continue;
                        ...
                        checkdataret = check_data(
                            pattern, PATTERN_LEN * 2, 
                            pins[tck], pins[tdi], pins[tdo], 
                            &reg_len
                        ); 
 ...
 */
static void loopback_check(void)
{
    int tdo = 0, tdi = 0;
    int checkdataret = 0, reg_len = 0;
 ...
 */
static void scan_idcode(void)
{
    int tck = 0, tms = 0, tdo = 0, tdi = 0, ntrst = 0;
    int i = 0, j = 0, tdo_read = 0;
    int nr = 0; // number of devices
    uint32_t idcodes[MAX_DEV_NR], idcode = 0;
    
    // TODO: artifact that might need to be configurable
    char idcodestr[] = "                                ";
    int idcode_i = 31;
...
static void shift_bypass(void)
{
    int tdi = 0, tdo = 0, tck = 0;
    int checkdataret = 0, reg_len = 0;
...
static void brute_ir(int iterations, int tck, int tms, int tdi, int tdo, int ntrst = IGNOREPIN)
{
    ...
    for (uint32_t ir = 0; ir < (1UL << IR_LEN); ir += 1) 
    { 
        iractive = 0;
...
void set_pattern(void)
{
    ...
    while (1) 
    {
        ...
        case '.': 
            ...
            // bah. for the arduino serial console which does not pass us \n
            pattern[i] = 0;
 ...
 */
void print_prog_str(const char *str)
{
    char c = '\0';
...
/*
 * Read user command form serial port.
 */
int read_command(void)
{
    int i = 0, c = 0;

    // receive command bytes
    while (i < CMDLEN - 1)
    { 
        if (Serial.available())
        {
...
void loop() 
{
    ...
    // hoping read buffer is idle after 5 ms
    delay(5); 

    // EXECUTE COMMAND
    if (!strcmp(command, "s") || !strcmp(command, "pattern scan"))
    {
        scan();
    }
    else if (!strcmp(command, "1") || !strcmp(command, "pattern scan single")) 
    {
        int dummy = 0;
        ...
        if (check_data(pattern, PATTERN_LEN * 2, pins[TCK], pins[TDI], pins[TDO], &dummy))
        {
    ...
    else if (!strcmp(command, "p") || !strcmp(command, "pattern set"))
    {
        set_pattern();
    }
    else if (!strcmp(command, "l") || !strcmp(command, "loopback check"))
    {
        loopback_check();
    }
    else if (!strcmp(command, "i") || !strcmp(command, "idcode scan"))
    {
        scan_idcode();
    }
    else if (!strcmp(command, "b") || !strcmp(command, "bypass scan"))
    {
        shift_bypass();
    }
    else if (!strcmp(command, "x") || !strcmp(command, "boundary scan"))
    {
        Serial.print("pins");
    ...
    else if (!strcmp(command, "y") || !strcmp(command, "irenum"))
    {
    ...
    else if (!strcmp(command, "d") || !strcmp(command, "delay"))
    {
        DELAY = ~DELAY;

        Serial.println(DELAY ? "Delay ON" : "Delay OFF");
    }
    else if (!strcmp(command, "-") || !strcmp(command, "delay -"))
    {
    ...
    else if (!strcmp(command, "r") || !strcmp(command, "pullups"))
    {
        PULLUP = ~PULLUP;
